...
func Usage() {
	...
	fmt.Fprintln(os.Stderr, "  Rsp do_lock(Req req)")
	fmt.Fprintln(os.Stderr, "  Rsp un_lock(Req req)")
	fmt.Fprintln(os.Stderr, "  Rsp client_states()")
	fmt.Fprintln(os.Stderr)
	os.Exit(0)
}
...
func main() {
	...
	client := example.NewGetLockClientFactory(trans, protocolFactory)
	...
	case "do_lock":
		if flag.NArg()-1 != 1 {
			fmt.Fprintln(os.Stderr, "DoLock requires 1 args")
			flag.Usage()
		}
		arg14 := flag.Arg(1)
		...
		value0 := argvalue0
		fmt.Print(client.DoLock(value0))
		fmt.Print("\n")
		break
	case "un_lock":
		if flag.NArg()-1 != 1 {
	...
	case "client_states":
		if flag.NArg()-1 != 0 {
			fmt.Fprintln(os.Stderr, "ClientStates requires 0 args")
			flag.Usage()
		}
		fmt.Print(client.ClientStates())
		fmt.Print("\n")
		break
	case "":
		Usage()
